/**
 * tech-requests.js — Entry point (slim orchestrator)
 *
 * Only exports: renderTechRequests(container)
 * Form modals are now in:
 *  - tech-request-form.js → openModal, openEditModal
 */
...
function injectTabStyles() {
  if (document.getElementById('tr-tab-styles')) return;
  ...
  st.textContent = `
    ...
    #tr-modal input[type="text"], #tr-modal input[type="number"], #tr-modal textarea, #tr-modal select {
    ...
    #tr-modal input[type="text"]:focus, #tr-modal input[type="number"]:focus,
    #tr-modal textarea:focus, #tr-modal select:focus {
      border-color: rgba(99,102,241,0.7) !important; outline: none;
      box-shadow: 0 0 0 3px rgba(99,102,241,0.18); background: rgba(99,102,241,0.07) !important;
    }
    #tr-modal input::placeholder, #tr-modal textarea::placeholder { color: rgba(200,200,220,0.35); }
    ...
    .tr-type-card { display:flex;align-items:center;gap:12px;cursor:pointer;padding:14px 16px;
      border:2px solid rgba(255,255,255,0.1);border-radius:12px;flex:1;transition:all .2s;background:rgba(255,255,255,0.03); }
    ...
    .tr-section { margin-top:20px;padding-top:16px;border-top:1px solid rgba(255,255,255,0.07); }
    .tr-section-title { font-size:11px;font-weight:700;color:#6366f1;text-transform:uppercase;
      letter-spacing:.08em;margin-bottom:12px;display:flex;align-items:center;gap:8px; }
    .tr-section-title::after { content:'';flex:1;height:1px;background:linear-gradient(90deg,rgba(99,102,241,.3),transparent); }
    .tr-item-row { display:grid;grid-template-columns:2fr 58px 1fr 30px 30px;gap:6px;margin-bottom:8px;
      align-items:center;background:rgba(255,255,255,.03);padding:6px 8px;border-radius:8px;
      border:1px solid rgba(255,255,255,.06);transition:border-color .2s; }
    .tr-item-row:hover { border-color:rgba(99,102,241,.25); }
    #tr-btn-add-item { background:rgba(99,102,241,.12);border:1px dashed rgba(99,102,241,.45);color:#818cf8;
      border-radius:8px;padding:6px 16px;font-size:12px;cursor:pointer;font-weight:600;transition:all .2s; }
    #tr-btn-add-item:hover { background:rgba(99,102,241,.2);border-color:rgba(99,102,241,.7);color:#a5b4fc; }
    .tr-modal-footer { display:flex;gap:10px;justify-content:flex-end;margin-top:24px;
...
function renderShell() {
  return `
    ...
    <div id="tr-modal-overlay" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.7);backdrop-filter:blur(4px);z-index:1000;align-items:flex-start;justify-content:center;padding:20px;overflow-y:auto;overflow-x:hidden;">
      <div id="tr-modal" style="border-radius:16px;padding:32px;width:min(640px,95vw);margin:0 auto;"></div>
    </div>
  `;
}

function renderTableContent(data, activeTab) {
        ...
        ${data.requests.map(r => `
          <div class="tr-card tr-row" data-id="${r.id}">
            ...
            <div class="tr-card-extra">
              ${isInc ? (r.equipment_name || '—') : `${r.quantity} equipo${r.quantity !== 1 ? 's' : ''}`}
            </div>
...
function bindTableEvents(container2, loadTable, setPage) {
...
export async function renderTechRequests(container) {
  ...
  async function loadTable() {
    ...
    const params = new URLSearchParams({ type: activeTab, page: currentPage, limit, ...filters });
    try {
      ...
      container2.innerHTML = renderTableContent(data, activeTab);
      bindTableEvents(container2, loadTable, (p) => { currentPage = p; });
    } catch (err) {
