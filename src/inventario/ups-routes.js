...
router.get('/api/inventario/ups', ...canRead, wrap(async (req, res) => {
  ...
  const { total } = await db.prepare(`SELECT COUNT(*) AS total FROM inventario_ups ${wc}`).get(...params);
...
router.post('/api/inventario/ups', ...canCreate, wrap(async (req, res) => {
  const { placa, marca, nombre_equipo, modelo, serial, area, voltaje, fecha_compra, fecha_despacho, ciudad, nombre_completo, cedula } = req.body;
  if (!placa?.trim()) return res.status(400).json({ error: 'placa es requerida.' });
  const qr_token = randomUUID();
  try {
    const result = await db.prepare(
      `INSERT INTO inventario_ups (placa,marca,nombre_equipo,modelo,serial,area,voltaje,fecha_compra,fecha_despacho,qr_token,ciudad,nombre_completo,cedula) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)`
    ).run(placa.trim(), marca||null, nombre_equipo||null, modelo||null, serial||null, area||null, voltaje||null, fecha_compra||null, fecha_despacho||null, qr_token, ciudad||null, nombre_completo||null, cedula||null);
...
router.put('/api/inventario/ups/:id', ...canEdit, wrap(async (req, res) => {
  ...
  if (!await db.prepare('SELECT id FROM inventario_ups WHERE id=?').get(id)) return res.status(404).json({ error: 'UPS no encontrada.' });
  const { placa, marca, nombre_equipo, modelo, serial, area, voltaje, fecha_compra, fecha_despacho, ciudad, nombre_completo, cedula } = req.body;
  try {
    await db.prepare(
      `UPDATE inventario_ups SET placa=?,marca=?,nombre_equipo=?,modelo=?,serial=?,area=?,voltaje=?,fecha_compra=?,fecha_despacho=?,ciudad=?,nombre_completo=?,cedula=?,updated_at=datetime('now','localtime') WHERE id=?`
    ).run(placa, marca||null, nombre_equipo||null, modelo||null, serial||null, area||null, voltaje||null, fecha_compra||null, fecha_despacho||null, ciudad||null, nombre_completo||null, cedula||null, id);
    res.json({ ok: true });
  } catch (err) {
...
router.delete('/api/inventario/ups/:id', ...canDelete, wrap(async (req, res) => {
  const result = await db.prepare('DELETE FROM inventario_ups WHERE id=?').run(parseInt(req.params.id));
