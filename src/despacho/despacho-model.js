...
export async function getDespachos(db, { search, requiere_acta, acta_firmada, limit = 20, offset = 0 } = {}) {
  ...
  const total = (await db.prepare(query.replace('SELECT *', 'SELECT COUNT(*) as n')).get(...params)).n;
  const rows  = await db.prepare(`${query} ORDER BY id DESC LIMIT ? OFFSET ?`).all(...params, parseInt(limit), parseInt(offset));
...
export async function patchDespacho(db, id, fieldMap) {
  ...
  await db.prepare(`UPDATE despachos SET ${setClauses} WHERE id = ?`).run(...values, id);
}
...
export async function getTiposArticulo(db) {
  const rows = await db.prepare("SELECT id, nombre FROM tipos_articulo WHERE activo = 1 ORDER BY nombre ASC, id ASC").all();
  const seen = new Set();
  return rows.filter(r => { if (seen.has(r.nombre)) return false; seen.add(r.nombre); return true; });
}
...
export async function upsertTipoArticulo(db, nombre) {
  ...
  const result = await db.prepare("INSERT INTO tipos_articulo (nombre) VALUES (?)").run(nombre);
