...
export default function UpsFormModal({ row, onClose, onSaved }) {
  ...
  async function submit(e) {
    ...
    try {
      const res = await fetch(`/api/inventario/ups/${row.id}`, {
  ...
  return (
    <div style={{position:'fixed',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,.6)',zIndex:10000}}>
      <div style={{background:'#fff',padding:20,borderRadius:8,width:'min(560px,96vw)',maxHeight:'90vh',overflowY:'auto'}}>
        ...
        <form onSubmit={submit}>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
            ...
            <div>
              <label style={{display:'block',fontSize:12}}>Nombre del equipo</label>
              <input name="nombre_equipo" value={form.nombre_equipo} onChange={onChange} />
            </div>
            <div>
            ...
            <div>
              <label style={{display:'block',fontSize:12}}>Voltaje</label>
              <input name="voltaje" value={form.voltaje} onChange={onChange} />
            </div>
          </div>
