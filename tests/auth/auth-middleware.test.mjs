...
const mockGetSession = mock.fn(async (token) => {
...
test('requireAuth: no cookie → 401', async () => {
  const res = makeRes(); let called = false;
  await requireAuth(makeReq(''), res, () => { called = true; });
  assert.equal(res._status, 401);
  assert.equal(called, false);
});

test('requireAuth: invalid token → 401', async () => {
  const res = makeRes(); let called = false;
  await requireAuth(makeReq('it_session=bogus'), res, () => { called = true; });
  assert.equal(res._status, 401);
  assert.equal(called, false);
});

test('requireAuth: valid token → sets req.user and calls next', async () => {
...
test('requireAuth: parses token from cookie with other fields', async () => {
